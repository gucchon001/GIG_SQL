...
# ログ動作確認用メッセージ
print("=" * 80)
print("subcode_loader.py: ログシステム初期化完了")
print("=" * 80)
LOGGER.debug("=" * 80)
LOGGER.info("=" * 80)
LOGGER.warning("=" * 80)
LOGGER.error("=" * 80)
LOGGER.debug("subcode_loader.py: ログシステム初期化完了（DEBUGレベル）")
LOGGER.info("subcode_loader.py: ログシステム初期化完了（INFOレベル）")
LOGGER.warning("subcode_loader.py: ログシステム初期化完了（WARNINGレベル）")
LOGGER.error("subcode_loader.py: ログシステム初期化完了（ERRORレベル）")
LOGGER.debug("=" * 80)
LOGGER.info("=" * 80)
LOGGER.warning("=" * 80)
LOGGER.error("=" * 80)
...
def write_to_log_sheet(csv_file_name_column, sheet_name, main_table_name, category, record_count, json_keyfile_path, result, error_log=None, save_path_id=None):
    ...
    try:
        ...
        # ログデータの型チェック
        LOGGER.debug("ログシート書き込みデータの型チェック:")
        for idx, item in enumerate(row_data):
            item_type = type(item).__name__
            LOGGER.debug(f"  項目{idx+1}: {item_type} = {str(item)[:100]}")
            if item_type not in ['str', 'int', 'float', 'bool', 'NoneType']:
                LOGGER.warning(f"ログデータで想定外の型が検出: 項目{idx+1}: {item_type} = {item}")
